...
import { GROUP_EDIT } from "../../../Constants/constant";
...
const EditGroup = () => {
    ...
    const createGroup = () => {
        ...
        if(!groupValid || !colorValid || !messageValid || !noteValid || !taxValid || !depositValid || !approvalValid || !offlineValid || !onlineValid){
            ...
            apis.post(`/supplier/group/${id}/update`, bodyData, config)
            .then((res) => {
                if(res.data.success === true){
                    toast.success("Group updated successfully.", {autoClose: 2000, position: toast.POSITION.TOP_CENTER,});
                    navigate('/supplier/groups-management')
                }else{
                    toast.error("Could not update group. Please try again later.", {autoClose: 2000, position: toast.POSITION.TOP_CENTER,});
                }
            })
            .catch((error) => {
    ...
    useEffect(() => {
        ...
        apis.get(`/supplier/group/${id}`, config)
        .then((res) => {
            if(res.data.success === true){
                setGroupName(res.data.data.name)
                setColour(res.data.data.color)
                setMessage(res.data.data.order_confirm_msg)
                setNote(res.data.data.order_default_note)
                setTax(res.data.data.tax_applicability)
                setBill(res.data.data.bill_deposits)
                setApproval(res.data.data.order_approval)
                setIsCount(res.data.data.is_min_order_count === 1 ? true : false)
                if(res.data.data.is_min_order_count === 1){
                    setMinItem(res.data.data.min_items)
                    setMinKeg(res.data.data.min_kegs)
                }
                setIsValue(res.data.data.is_min_order_value === 1 ? true : false)
                if(res.data.data.is_min_order_value === 1){
                    setMinPrice(res.data.data.min_price)
                }
                setPayment(res.data.data.is_accepted_payment === 1 ? true : false)
                if(res.data.data.is_accepted_payment === 1){
                    setOnline(res.data.data.online_payment)
                    setOffline(res.data.data.offline_payment)
                }


            }else{
                toast.error("Something went wrong. Please try again later.", {autoClose: 2000, position: toast.POSITION.TOP_CENTER,});
            }
        })
        .catch((error) => {
            if(error.message !== "revoke"){
                        ...
                        <div class="row">
                            <div class="col-12 mb-3">
                                    ...
                                    <div class="card-body p-4">
                                        <div class="row">
                                            <div class="col-sm-8 col-12"> 
                                                    ...
                                                    <div className="mt-4"> 
                                                        <div class="form-check form-check-inline">
                                                            <input class="form-check-input" type="checkbox" id="MaximumOrder" checked={isCount} onChange={(e) => setIsCount(e.target.checked)}/>
                                ...
                                <div class="d-flex">
                                    ...
                                    {hasPermission(GROUP_EDIT) && <button type="button" class="btn btn-purple width-auto" onClick={() => createGroup()}>Update</button>}
                                </div>
                                </form>
                            </div>
                        </div>
