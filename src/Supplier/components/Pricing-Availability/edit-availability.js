...
const EditAvailability = () => {
  ...
  const handleAddAvailability = () => {
    ...
    if (
      ...
      apis
        .post(`/supplier/availability/${id}/update`, bodyData, config)
        .then((res) => {
  ...
  useEffect(() => {
    if(getData){
      ...
      apis
        .get(`/supplier/availability/${id}`, mainConfig)
        .then((res) => {
          setProductName(res.data.data[0].product.product_name)
          setSelectedVisibility(res.data.data[0].visibility_id);
          // setLimited(res.data.data[0].)
          const arrFilter1 = editableGroups.filter((ele) => {
            return !res.data.data[0].allocations.some((elem) => elem.group_id === ele.id)
          })
          setEditableGroups(arrFilter1)
          if(res.data.data[0].allocations.length > 0){
            let newArr = []
            for(let i = 0; i < res.data.data[0].allocations.length; i++){
              let myObj = {
                group_id: res.data.data[0].allocations[i].group_id,
                group_name: res.data.data[0].allocations[i].group.name,
                quantity: res.data.data[0].allocations[i].allocation_quantity
              }
              newArr.push(myObj)
            }
            setList1Dummy(newArr)
          }else{
            setList1Dummy([])
          }

          const arrFilter2 = editableGroups2.filter((ele) => {
            return !res.data.data[0].maximums.some((elem) => elem.group_id === ele.id)
          })
          setEditableGroups2(arrFilter2)

          if(res.data.data[0].maximums.length > 0){
            let newArr = []
            for(let i = 0; i < res.data.data[0].maximums.length; i++){
              let myObj = {
                group_id: res.data.data[0].maximums[i].group_id,
                group_name: res.data.data[0].maximums[i].group.name,
                quantity: res.data.data[0].maximums[i].maximum_quantity
              }
              newArr.push(myObj)
            }
            setList2Dummy(newArr)
          }else{
            setList2Dummy([])
          }
          setTargetProduct(res.data.data[0].product_id)
        })
        .catch((error) => {
          console.log(error);
        });
